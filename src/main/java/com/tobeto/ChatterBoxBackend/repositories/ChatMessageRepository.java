...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Integer> {
    ...
    @Query("select new com.tobeto.ChatterBoxBackend.services.dtos.message.responses.GetAllMessagesResponse(" +
            "m.id, m.content, " +
            "new com.tobeto.ChatterBoxBackend.services.dtos.user.responses.GetUserByIdResponse(s.id, s.userName, s.email, s.firstname, s.lastname, s.image), " +
            "new com.tobeto.ChatterBoxBackend.services.dtos.user.responses.GetUserByIdResponse(r.id, r.userName, r.email, r.firstname, r.lastname, r.image), " +
    ...
    @Query("select distinct new com.tobeto.ChatterBoxBackend.services.dtos.user.responses.GetAllUsersResponse(" +
            "u.id, u.userName, u.email, u.firstname, u.lastname, u.image) " +
            "from Message m " +
            "join m.sender u " +
            "where m.recipient.id = :userId " +
            "union " +
            "select distinct new com.tobeto.ChatterBoxBackend.services.dtos.user.responses.GetAllUsersResponse(" +
            "u.id, u.userName, u.email, u.firstname, u.lastname, u.image) " +
            "from Message m " +
            "join m.recipient u " +
